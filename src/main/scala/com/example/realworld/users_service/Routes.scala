...
import com.example.realworld.users_service.users.{User, UsersService}
  ...
  val routes: Route = Route.seal(
    concat(
      ...
      path("user") {
        pathEndOrSingleSlash {
          get {
            authenticateOAuth2Async[User](jwtService.jwtIssuer, authenticator) {
              user =>
                complete(StatusCodes.OK,
                         UserDto.fromUserAndToken(
                           user,
        ...
        jwtService.decodeToken(token) match {
          case Right(email) =>
            usersService.getUserByEmail(email).map {
              ...
              case Left(exception) =>
                system.log.error("Unexpected exception when getUserByEmail",
                                 exception)
                None
            }
          case Left(exception) =>
            system.log.error("Unexpected exception when decodeToken", exception)
